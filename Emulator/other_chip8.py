...
class Emulator:
    ...
    def execution(self):
        ...
        # self.log.append({
        #     'op':'0x'+opcode,
        #     'pc':"{0:#0{1}x}".format(self.ProgramCounter,6),
        #     'v':["{0:#0{1}x}".format(v.value,4) for v in self.Registers],
        #     'i':"{0:#0{1}x}".format(self.IRegister.value,6)
        # })

        if len(self.log) > 980:
            #
            #
            # print('\n'.join(['0x'+l for l in self.log]))
            #
            for l in self.log:
                print('PC[{}] OP[{}] I[{}] V[{}]'.format(l['pc'], l['op'], l['i'], ', '.join(l['v'])))
            self.log = []
                # print('{}: PC[{}] OP[{}] I[{}] V[{}]'.format(index, hex(l['pc']), l['op'], hex(l['i']), ', '.join([hex(a) for a in l['v']])))
